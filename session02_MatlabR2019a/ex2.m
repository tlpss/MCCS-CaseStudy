...
classdef ex2
    ...
    methods(Static)
        ...
        function [Q1,Q2] = getLQRCostFunctArrays()
            ...
            Q1 = zeros(5);
            Q1(1,1) = 0.00001;
            Q1(2,2) = 50; % very large penalty for high distance to path value (x2)
            Q1(3,3) = 0.5;
            Q1(4,4) = 0.5;
            Q1(5,5) = 0.5;
        ...
        function [LQR_gain, SInf, singular_values] = getLQRGain(Phi,Gamma,Q1,Q2)
            ...
            for i=1:n_iterations
                ...
                %%- Ricatti Equation 
                S=transpose(Phi)*(S-S*Gamma*inv(Q2 + transpose(Gamma)*S*Gamma)*transpose(Gamma)*S)*Phi + Q1;
            ...
            LQR_gain = inv(Q2 + transpose(Gamma)*S*Gamma)*transpose(Gamma)*S*Phi;
            SInf=S;
        ...
        function n_states_not_observable = checkObservability(Phi,Cprime)
            ...
            n_states_not_observable = size(Phi,1)- N_obs;
        end
        %
        function [L, selected_poles] = getObserverGain(Phi, Gamma, lqr_K, Cprime)
            ...
            % x_hat(k+1) = (Phi) x_hat(k) - L*Cprime x_hat(k)
            % so A = Phi and B = Cprime
            % HOWEVER: this is A-K*B whereas poles uses A-B*K
            % -> transpose the control loop -> L' 
            %https://nl.mathworks.com/help/control/getstart/pole-placement.html
        ...
        function test_LQR()
            params = ex1.getSystemParameters();
            [A,B,C,D] = ex1.getLinealModelArrays(params);
            [Phi,Gamma] = ex1.getDiscreteLinearModel(A,B,C,D,0.1,'c2d');
            
            [Q1,Q2] = ex2.getLQRCostFunctArrays();
            
            k = ex2.getLQRGain(Phi,Gamma,Q1,Q2)
            k_opt = dlqr(Phi,Gamma,Q1,Q2)
        end
        ...
        function test_Obs()
            ...
            [A,B,C,D] = ex1.getLinealModelArrays(params);
            [Phi,Gamma] = ex1.getDiscreteLinearModel(A,B,C,D,0.1,'c2d');
            Cprime = ex2.alternativeSystemsOutputEquation(C)
            ex2.checkObservability(Phi,Cprime)
        end 
    end
end
